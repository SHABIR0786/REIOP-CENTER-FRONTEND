            ...
            <b-row class="mb-1 text-center">
                <b-col cols="12">
                    <b-input-group prepend="Type In File" class="mb-2" id="type_in_file" label="Type In File" label-for="type_in_file">
                            <b-form-input :state="validateState('type_in_file')" aria-describedby="type_in_file" type="text" v-model="$v.subjectTypes.type_in_file.$model" required></b-form-input>
            ...
            <b-row class="mb-1 text-center">
                <b-col cols="12">
                     <b-input-group prepend="Type In Web" class="mb-2" id="type_in_web" label="Type In Web" label-for="type_in_web">
                            <b-form-input :state="validateState('type_in_web')" aria-describedby="type_in_web" type="text" v-model="$v.subjectTypes.type_in_web.$model" required></b-form-input>
...
export default {
    ...
    methods: {
        validateState(name) {
            const {$dirty,$error} = this.$v.subjectTypes[name];
            return $dirty ? !$error : null;
        },
        onSubmit() {
            this.$v.subjectTypes.$touch();
            if (this.$v.subjectTypes.$anyError) {
                return;
            }
