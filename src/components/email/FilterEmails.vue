...
export default {
   ...
   async showModal() {
      if (this.showModal){
        try{
          ...
          let response = await this.$store.dispatch("emailModule/emailFilterList", {filter: this.allFilters, search: this.search});
  ...
  methods: {
    ...
    MapFilters(response) {
      ...
      if(response?.email_error_type?.length > 0) {
      ...
      if(response?.email_skip_source?.length > 0){
        response.email_skip_source.forEach(el=>{
