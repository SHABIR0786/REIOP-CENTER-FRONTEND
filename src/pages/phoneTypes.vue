...
import AddTypeModal from "../components/phoneTypes/AddTypesModal";
import EditTypeModal from "../components/phoneTypes/EditTypeModal";
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      fields: 'phoneTypesModule/fields',
      items: 'phoneTypesModule/types',
      total: 'phoneTypesModule/total',
    }),
    rows() { return this.total ? this.total : 1 }
  },
  async created () {
    this.$store.dispatch('uxModule/setLoading')
    this.$store.dispatch('phoneTypesModule/getTotal')
    try {
      await this.$store.dispatch("phoneTypesModule/getAllTypes", {page: 1, perPage: this.perPage})
  ...
  methods: {
    ...
    save(item) {
      ...
      this.$store.dispatch('phoneTypesModule/editType', {...item})
    },
    add(item) {
      this.showAddModal = false
      this.$store.dispatch('phoneTypesModule/addType', {...item})
    },
    deleteItem(item){
    ...
    modalResponse(response) {
      ...
      if (response) {
        this.$store.dispatch('phoneTypesModule/deleteType', this.itemToDelete.id)
      }
    },
  },
  mounted() {},
  watch: {
    currentPage: {
      handler: function() {
        this.$store.dispatch('phoneTypesModule/getAllTypes', {page: this.currentPage, perPage: this.perPage, search: this.search})
      }
    },
    perPage: {
      handler: function () {
        this.$store.dispatch('phoneTypesModule/getAllTypes', {page: 1, perPage: this.perPage, search: this.search})
      }
    },
    search: {
      handler: function () {
        this.$store.dispatch('phoneTypesModule/searchType', {page: this.currentPage, perPage: this.perPage, search: this.search})
      }
    }
  }
}
