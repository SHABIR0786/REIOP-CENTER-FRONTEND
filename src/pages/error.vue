...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      fields: 'errorModule/fields',
      items: 'errorModule/subjects',
      total: 'errorModule/total'
    }),
    rows() { return this.total ? this.total : 1}
  },
  async created () {
    this.$store.dispatch('errorModule/getTotal')
    try {
      this.$store.dispatch('uxModule/setLoading')
      await this.$store.dispatch("errorModule/getAllSubjects", {page: 1, perPage: this.perPage})
  ...
  methods: {
    ...
    save(item) {
      ...
      this.$store.dispatch('errorModule/editSubject', {...item})
    },
    add(item) {
      this.showAddModal = false
      this.$store.dispatch('errorModule/addSubject', {...item})
    },
    deleteSubject(item){
    ...
    modalResponse(response) {
      ...
      if (response) {
        this.$store.dispatch('errorModule/deleteSubject', this.itemToDelete.id)
      }
    },
    addItem() {
  ...
  watch: {
    currentPage: {
      handler: function() {
        this.$store.dispatch('errorModule/getAllSubjects', {page: this.currentPage, perPage: this.perPage, search: this.searchSubject})
      }
    },
    perPage: {
      handler: function () {
        this.$store.dispatch('errorModule/getAllSubjects', {page: 1, perPage: this.perPage, search: this.searchSubject})
      }
    },
    searchSubject: {
      handler: function () {
        this.$store.dispatch('errorModule/searchSubjects', {page: this.currentPage, perPage: this.perPage, search: this.searchSubject})
      }
    }
  }
}
