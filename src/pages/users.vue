            ...
            <b-row>
                <b-col cols="12" class="d-flex justify-content-end">
                    <b-button variant="primary" class="add-seller" @click="addItem()">
                        <b-icon icon="plus" aria-hidden="true"></b-icon> Add User</b-button>
                </b-col>
            </b-row>
            <hr>
            <b-row class="mb-3">
...
export default {
    ...
    async created () {
        ...
        try {
         ...
         await this.$store.dispatch("userModule/getAllUsers", {page: 1, perPage: this.perPage, search: this.searchUser, sortBy:this.sortBy, sortDesc:this.sortDesc})
    ...
    methods: {
       ...
       async search() {
            await this.$store.dispatch('userModule/searchUsers', { page: this.currentPage, perPage: this.perPage, search: this.searchUser, sortBy:this.sortBy,sortDesc:this.sortDesc })
            this.itemsCount = this.total;
            this.filteredOrAllData = this.items;
       ...
       async sortingChanged(ctx) {
        ...
        save(item) {
            ...
            this.$store.dispatch('userModule/editUser', {...item})
        },
        async add(item) {
            this.showAddModal = false
            await this.$store.dispatch('userModule/addUser', {...item});
        },
        deleteUser(item) {
        ...
        bulkDelete () {
            this.$store.dispatch('userModule/deleteMultipleUsers', this.bulkDeleteItems).then(() => {
              this.$store.dispatch('userModule/getAllUsers', {page: this.currentPage, perPage: this.perPage, search: this.searchUser, sortBy:this.sortBy,sortDesc:this.sortDesc})
            //   this.$store.dispatch('userModule/getTotal')
            })
        },
        selectAll () {
    ...
    watch: {
        currentPage: {
            handler: async function() {
                await this.$store.dispatch('userModule/getAllUsers', { page: this.currentPage, perPage: this.perPage, search: this.searchUser, sortBy:this.sortBy,sortDesc:this.sortDesc })
                this.filteredOrAllData = this.items
            }
        },
        perPage: {
