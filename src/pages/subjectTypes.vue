...
import AddTypeModal from "../components/subjectTypes/AddTypesModal";
import EditTypeModal from "../components/subjectTypes/EditTypeModal";
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      fields: 'subjectTypesModule/fields',
      items: 'subjectTypesModule/subjectTypes',
      total: 'subjectTypesModule/total',
    }),
    rows() { return this.total ? this.total : 1 }
  },
  async created () {
    this.$store.dispatch('uxModule/setLoading')
    this.$store.dispatch('subjectTypesModule/getTotal')
    try {
      await this.$store.dispatch("subjectTypesModule/getAllTypes", {page: 1, perPage: this.perPage})
  ...
  methods: {
    ...
    save(item) {
      ...
      this.$store.dispatch('subjectTypesModule/editType', {...item})
    },
    add(item) {
      this.showAddModal = false
      this.$store.dispatch('subjectTypesModule/addType', {...item})
    },
    deleteItem(item){
    ...
    modalResponse(response) {
      ...
      if (response) {
        this.$store.dispatch('subjectTypesModule/deleteType', this.itemToDelete.id)
      }
    },
  },
  mounted() {},
  watch: {
    currentPage: {
      handler: function() {
        this.$store.dispatch('subjectTypesModule/getAllTypes', {page: this.currentPage, perPage: this.perPage, search: this.search})
      }
    },
    perPage: {
      handler: function () {
        this.$store.dispatch('subjectTypesModule/getAllTypes', {page: 1, perPage: this.perPage, search: this.search})
      }
    },
    search: {
      handler: function () {
        this.$store.dispatch('subjectTypesModule/searchType', {page: this.currentPage, perPage: this.perPage, search: this.search})
      }
    }
  }
}
